...
import (
	...
	"google3/security/endpoint_identity_ca/lib/acme/server/testing/testkeys"
...
func setupTestCert(t *testing.T, modifyCertTemplate func(*testing.T, *x509.Certificate)) *x509.Certificate {
	...
	certBytes, err := x509.CreateCertificate(rand.Reader, template, parentCert, testkeys.ECPrivateKey().Public(), privateKey)
	if err != nil {
...
func createSubjectAltNameExtension(t *testing.T, san *x509ext.SubjectAltName, critical bool) pkix.Extension {
	t.Helper()
	marshalledSAN, err := x509ext.MarshalSubjectAltName(san, critical)
	if err != nil {
